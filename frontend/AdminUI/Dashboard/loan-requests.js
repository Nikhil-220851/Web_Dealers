...
function renderLoanRequests(loans) {
    ...
    if (!loans || loans.length === 0) {
        tbody.innerHTML = `<tr><td colspan="9"
            style="text-align:center;color:#64748b;padding:32px;">
...
function filterLoans() {
    ...
    const filtered = allLoans.filter(l => {
        ...
        const matchStatus = !status ||
            (l.status||'pending').toLowerCase() === status;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('searchInput')
        ?.addEventListener('input',  filterLoans);
    document.getElementById('statusFilter')
        ?.addEventListener('change', filterLoans);
});