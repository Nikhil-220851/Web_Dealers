/* ═══════════════════════════════════════
   LoanPro Bank Employee Dashboard — dashboard.js
═══════════════════════════════════════ */
...
async function fetchDashboardData() {
    try {
        const res = await fetch(`${API_BASE}/emp-get-dashboard.php`);
        ...
        if (result.status === 'success') {
            ...
            // Populate KPI Cards
            document.getElementById('statTotalAssigned').textContent = safeValue(data.total_assigned);
            document.getElementById('statPendingVerifications').textContent = safeValue(data.pending_verifications);
            document.getElementById('statVerifiedLoans').textContent = safeValue(data.verified_loans);
    ...
    container.innerHTML = `
        <div class="pending-table-wrap">
            <table class="pending-table">
                <thead><tr>
                    <th>Loan ID</th><th>Loan Amount</th><th>Status</th><th>Verification</th>
                </tr></thead>
                <tbody>
                    ${list.map(item => {
                        ...
                        <tr class="pending-row">
                            ...
                            <td>${escapeHtml(item.verification_status.toUpperCase())}</td>
                        </tr>
                    `}).join('')}
                </tbody>
            </table>
