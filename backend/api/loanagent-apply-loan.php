...
try {
    ...
    if ($borrower) {
        ...
        // Update borrower fields just in case they provided new email/address
        ...
        $newUser = [
            ...
            'role' => 'borrower',
            'created_at' => $now,
            'password' => password_hash($b_phone, PASSWORD_DEFAULT) // default auth mechanism
        ];
    ...
    $database->kyc_details->updateOne(
        ...
        ['$set' => [
            ...
            'pan_number' => $k_pan,
            'status' => 'verified', // Auto verified for agent
            'updated_at' => $now
        ]],
        ['upsert' => true]
    );
    ...
    $loanDoc = [
        "user_id"           => $userId, // Maintained for backwards compatibility with LMS_Web
        "borrower_id"       => $userId,
