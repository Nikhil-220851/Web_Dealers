...
try {
    // Check duplicate email in admins collection
    $existing = $database->admins->findOne(['email' => $email]);
    if ($existing) {
        http_response_code(409);
        echo json_encode(["status" => "error", "message" => "An admin account with this email already exists"]);
        exit();
    }
    ...
    $result = $database->admins->insertOne($adminDocument);
    ...
    if ($result->getInsertedCount() === 1) {
        ...
        echo json_encode([
            ...
            "adminId" => (string) $result->getInsertedId()
        ]);
    } else {
        http_response_code(500);
        echo json_encode(["status" => "error", "message" => "Failed to create admin account. Please try again."]);
    }
